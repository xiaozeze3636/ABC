...
class BERTEmbedding(nn.Module):
    def __init__(self, d_model, dropout=0.1, add_time_in_day=False, add_day_in_week=False,
        ...
        # 多尺度小波变换相关参数
        self.num_wavelet_features = 24  # 每个尺度的小波滤波器数量
        self.wavelet_kernel_sizes = [3, 5, 7]  # 不同尺度的小波卷积核大小
        self.num_scales = len(self.wavelet_kernel_sizes)

        # 定义多个可学习的小波卷积层
        self.wavelet_convs = nn.ModuleList([
            nn.Conv1d(
                in_channels=1,
                out_channels=self.num_wavelet_features,
                kernel_size=ks,
                padding=ks // 2,
                dilation=1,  # 可以根据需要调整膨胀率以捕捉不同的时间跨度
                bias=False
            )
            for ks in self.wavelet_kernel_sizes
        ])

        # 定义可学习的权重矩阵，将多尺度小波系数映射到 d_model 维度
        self.weight_wavelets = nn.Parameter(torch.randn((self.num_scales, self.num_wavelet_features, d_model)))
    ...
    def forward(self, sequence, position_ids=None, graph_dict=None):
        if self.add_gat:
            x = self.token_embedding(
                ...
                x=sequence[:, :, 0]  # 假设第一个特征是用于GAT的特征
            )  # (B, T, d_model)
        else:
            # 如果不使用GAT层，初始化为零张量
            x = torch.zeros(sequence.size(0), sequence.size(1), self.d_model).to(sequence.device)

        if self.add_pe:
            x += self.position_embedding(x, position_ids)  # (B, T, d_model)

        if self.add_time_in_day:
            x += self.daytime_embedding(sequence[:, :, 2])  # 假设第三个特征是时间（分钟）

        if self.add_day_in_week:
            x += self.weekday_embedding(sequence[:, :, 3])  # 假设第四个特征是星期几

        # 提取时间特征
        time_feature = sequence[:, :, 2].float()  # (B, T), 假设时间以分钟为单位
        B, T = time_feature.size()

        # 归一化时间特征到 [0, 1]
        time_normalized = time_feature / 1440.0  # 因为一天有1440分钟

        # 重新调整形状以适应Conv1d输入 (B, 1, T)
        time_normalized = time_normalized.unsqueeze(1)  # (B, 1, T)

        # 应用多尺度小波卷积
        wavelet_features = []
        for scale, conv in enumerate(self.wavelet_convs):
            wavelet_coeffs = conv(time_normalized)  # (B, num_wavelet_features, T)
            # 转换形状为 (B, T, num_wavelet_features)
            wavelet_coeffs = wavelet_coeffs.permute(0, 2, 1)  # (B, T, num_wavelet_features)
            # 映射小波系数到 d_model 维度
            weighted_wavelet = torch.matmul(wavelet_coeffs, self.weight_wavelets[scale])  # (B, T, d_model)
            wavelet_features.append(weighted_wavelet)  # List of (B, T, d_model)

        # 将所有尺度的小波特征进行聚合（例如求和或拼接）
        weighted_wavelet_sum = torch.stack(wavelet_features, dim=0).sum(dim=0)  # (B, T, d_model)
        # 将加权后的多尺度波特征添加到原始特征上
        x = x + weighted_wavelet_sum  # (B, T, d_model)
...
class LinearETA(nn.Module):
    ...
    def _load_pretrain_model(self, pretrain_path):
        try:
            # 加载预训练模型的state_dict
            pretrained_model = torch.load(pretrain_path, map_location=torch.device('cpu'))  # 可以根据需要改为 'cuda'
            ...
            self._logger.error(f"Error loading pretrained model: {str(e)}")
...
class MI_Estimator(nn.Module):
    ...
    def forward(self, z1, z2, z3):
        """
        计算两个互信息评分：z1与z2，以及z1与z3。

        Args:
            z1 (torch.Tensor): MLM 表示，形状 [B, D]
            z2 (torch.Tensor): view1 表示，形状 [B, D]
            z3 (torch.Tensor): view2 表示，形状 [B, D]

        Returns:
            torch.Tensor: 两个互信息评分，形状 [B, 2]c
        """
        # 计算 z1 与 z2 的评分
        joint1 = torch.cat([z1, z2], dim=1)  # [B, 2D]
        h1 = self.activation(self.bn1(self.fc1(joint1)))
        h1 = self.activation(self.bn2(self.fc2(h1)))
        output1 = self.fc3(h1)  # [B, 1]

        # 计算 z1 与 z3 的评分
        joint2 = torch.cat([z1, z3], dim=1)  # [B, 2D]
        h2 = self.activation(self.bn1(self.fc1(joint2)))
        h2 = self.activation(self.bn2(self.fc2(h2)))
        output2 = self.fc3(h2)  # [B, 1]
...
class BERTContrastiveLM(nn.Module):
    ...
    def __init__(self, config, data_feature):
        ...
        # **直接在 init 中定义 self.device**
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

        # 假设 self.pooler 输出的维度是 d_model

        self.projection_head = ProjectionHead(self.d_model, hidden_dim=4096, output_dim=256)
        self.projection_head.to(self.device)


        # Predictor 网络（只在在线网络中使用）
        self.predictor = Predictor(self.d_model, hidden_dim=4096, output_dim=256)
        ...
        # # 添加动量编码器
        # self.momentum_encoder = MomentumEncoder(copy.deepcopy(self.bert), copy.deepcopy(self.projection_head),
        #                                         momentum=self.momentum)
        #
        # self.momentum_encoder.to(self.device)

        # # 添加对照队列
        # self.queue = ContrastiveQueue(queue_size=self.queue_size, embedding_dim=self.d_model, device=self.device)

        # 定义温度参数

        self.mi_estimator = MI_Estimator(input_dim=self.d_model)
        self.mi_estimator.to(self.device)

        # # 定义 beta 参数，控制互信息项的权重
    ...
    def forward(self, contra_view1, contra_view2, argument_methods1,
        ...
        # print("cout_view1", out_view1.shape)
        # print(out_view1.shape)#torch.Size([64, 64, 256])
        # 使用 [CLS] token 作为全局表示
        cls_out_view1 = out_view1[:, 0, :]  # 形状 [B, D]
        # cls_out_view1 = F.normalize(cls_out_view1, dim=1)  # 归一化

        proj_view1 = self.projection_head(cls_out_view1)  # 形状 [B, 256]
        proj_view1 = F.normalize(proj_view1, dim=1)  # 归一化

        # 通过预测头生成 p1
        pred_view1 = self.predictor(proj_view1)  # (B, 256)
        pred_view1 = F.normalize(pred_view1, dim=1)  # L2 归一化


        # print(pool_out_view1.shape)#torch.Size([64, 256]
        ...
        out_view2, hidden_states2, _ = self.bert(x=contra_view2, padding_masks=padding_masks2,
                                                 ...
                                                 output_hidden_states=output_hidden_states,
                                                 output_attentions=output_attentions)
        cls_out_view2 = out_view2[:, 0, :]  # [B, D]
        proj_view2 = self.projection_head(cls_out_view2)  # [B, 256]
        proj_view2 = F.normalize(proj_view2, dim=1)  # 只经过投影头
        pred_view2 = self.predictor(proj_view2)  # [B, 256]
        pred_view2 = F.normalize(pred_view2, dim=1)

        # 计算 SimSiam 损失
        # 计算 SimSiam 损失
        loss_simsiam = self.simsiam_loss(pred_view1, proj_view2) + self.simsiam_loss(pred_view2, proj_view1)
        loss_simsiam = loss_simsiam / 2.0


        # 使用主编码器编码 masked_input 进行 MLM
        bert_output, _, _ = self.bert(x=masked_input, padding_masks=padding_masks,
                                      batch_temporal_mat=batch_temporal_mat, graph_dict=graph_dict,
                                      output_hidden_states=output_hidden_states,
                                      output_attentions=output_attentions)
        MLM_predictions = self.mask_l(bert_output)  # (B, T, V)

        # 获取 MLM 的全局表示，可以使用 [CLS] token
        cls_bert_output = bert_output[:, 0, :]  # [B, D]

        # 3. 计算互信息估计
        # 计算正样本对的判别器输出
        T_pos1, T_pos2 = self.mi_estimator(cls_bert_output, pred_view1, proj_view2)  # [B, 2]

        # 计算负样本对（打乱 z2）的判别器输出
        shuffled_pred_view1 = pred_view1[torch.randperm(pred_view1.size(0))]
        shuffled_proj_view2 = proj_view2[torch.randperm(proj_view2.size(0))]
        T_neg1, T_neg2 = self.mi_estimator(cls_bert_output, shuffled_pred_view1, shuffled_proj_view2)  # [B, 2]


        epsilon = 1e-8

        # 使用 logsumexp 计算，增强数值稳定性
        mi_lower_bound1 = torch.mean(T_pos1[:, 0]) - (torch.logsumexp(T_neg1[:, 0], dim=0) - math.log(T_neg1.size(0)))
        mi_lower_bound2 = torch.mean(T_pos2[:, 0]) - (torch.logsumexp(T_neg2[:, 0], dim=0) - math.log(T_neg2.size(0)))



        mi_lower_bound = mi_lower_bound1 + mi_lower_bound2

        # 限制 mi_lower_bound 的最小值，防止过于负
        mi_lower_bound = torch.clamp(mi_lower_bound, min=-10.0, max=10.0)

        # 计算互信息下界


        # a = self.mask_l(bert_output)
        return loss_simsiam, MLM_predictions, mi_lower_bound
...
#         self.projection_head_k = copy.deepcopy(projection_head)
...
#c
#     @torch.no_grad()
#     def update_encoder_k(self, online_encoder, online_projection_head):
#         for param_q, param_k in zip(online_encoder.parameters(), self.encoder_k.parameters()):
#             param_k.data = param_k.data * self.momentum + param_q.data * (1. - self.momentum)
#         for param_q, param_k in zip(online_projection_head.parameters(), self.projection_head_k.parameters()):
#             param_k.data = param_k.data * self.momentum + param_q.data * (1. - self.momentum)
#
#     def forward(self, x, padding_masks, batch_temporal_mat, argument_methods, graph_dict, output_hidden_states, output_attentions):
#         out, hidden_states, _ = self.encoder_k(x=x, padding_masks=padding_masks,
#                                               batch_temporal_mat=batch_temporal_mat,
#                                               argument_methods=argument_methods, graph_dict=graph_dict,
#                                               output_hidden_states=output_hidden_states,
#                                               output_attentions=output_attentions)
#         cls_out = out[:, 0, :]  # 使用 [CLS] token 作为全局表示
#         proj = self.projection_head_k(cls_out)
#         return proj
