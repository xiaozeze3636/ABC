...
class BERTContrastiveLM(nn.Module):
    ...
    def forward(self, contra_view1, contra_view2, argument_methods1,
        ...
        pool_out_view1 = self.pooler(bert_output=out_view1, padding_masks=padding_masks1,
                                     hidden_states=hidden_states1)  # (B, D)

        pool_out_view1 = F.normalize(pool_out_view1, dim=1)  # 归一化

        # 应用投影头
        proj_view1 = self.projection_head(pool_out_view1)  # (B, 256)
        ...
        with torch.no_grad():
            ...
            out_view2, hidden_states2, _ = self.momentum_encoder.encoder_k(x=contra_view2, padding_masks=padding_masks2,
                                                                           ...
                                                                           output_attentions=output_attentions)
            pool_out_view2 = self.pooler(bert_output=out_view2, padding_masks=padding_masks2,
                                         hidden_states=hidden_states2)  # (B, D)
            pool_out_view2 = F.normalize(pool_out_view2, dim=1)  # 归一化

            # 应用投影头
            proj_view2 = self.projection_head(pool_out_view2)  # (B, 256)
            proj_view2 = F.normalize(proj_view2, dim=1)  # 归一化
        ...
        return pred_view1, proj_view2, MLM_predictions
