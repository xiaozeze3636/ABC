...
class BERT(nn.Module):
    ...
    def __init__(self, config, data_feature):
        ...
        self.n_layers = self.config.get('n_layers', 12)a
...
class BERTContrastiveLM(nn.Module):
    ...
    def __init__(self, config, data_feature):
        ...
        self.projection_head = ProjectionHead(self.d_model, hidden_dim=4096, output_dim=256,dropout=0.1)
        self.projection_head.to(self.device)


        # Predictor 网络（只在在线网络中使用）
        self.predictor = Predictor(self.d_model, hidden_dim=4096, output_dim=256,dropout=0.1)
    ...
    def forward(self, contra_view1, contra_view2, argument_methods1,
        ...
        # 使用动量编码器编码 contra_view2
        with torch.no_grad():
            proj_view2 = self.momentum_encoder(
                contra_view2,
                padding_masks2,
                batch_temporal_mat2,
                argument_methods2,
        ...
        # 计算互信息下界
        mi_lower_bound1 = torch.mean(T_pos1[:, 0]) - torch.log(torch.mean(torch.exp(T_neg1[:, 0])))
        mi_lower_bound2 = torch.mean(T_pos2[:, 0]) - torch.log(torch.mean(torch.exp(T_neg2[:, 0])))
        ...
        return pred_view1, proj_view2, MLM_predictions, mi_lower_bound
