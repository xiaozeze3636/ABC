...
class Predictor(nn.Module):
    def __init__(self, input_dim=256, hidden_dim=4096, output_dim=256):
        ...
        self.net = nn.Sequential(
            nn.Linear(input_dim, hidden_dim),
            nn.LayerNorm(hidden_dim),  # 使用 LayerNorm 替代 BatchNorm1d
            nn.ReLU(inplace=True),
            nn.Linear(hidden_dim, output_dim)
        )
...
class BERTContrastiveLM(nn.Module):
    ...
    def __init__(self, config, data_feature):
        ...
        self.predictor = Predictor(input_dim=256, hidden_dim=4096, output_dim=256)
        ...
        # 添加动量编码器
        self.momentum_encoder = MomentumEncoder(copy.deepcopy(self.bert), momentum=self.momentum)
        ...
        # 添加线性投影层，将 feat_dim=5 投影到 d_model=768
    ...
    def forward(self, contra_view1, contra_view2, argument_methods1,
        ...
        #                               output_hidden_states=output_hidden_states,
        #                               output_attentions=output_attentions)  # (B, T, d_model)
        # 使用主编码器编码contra_view1

        # print("contra_view1:", contra_view1.shape)
        # print("contra_view2:", contra_view1.shape)
        # print(batch_temporal_mat1.shape)
        # contra_view1,contra_view2

        # contra_view2 = replace_elements(contra_view1, contra_view2, replace_prob=0.6)
        ...
        # print(f"contra_view1 shape: {contra_view1.shape}")
        # print(f"masked_input shape: {masked_input.shape}")
        # contra_view1
        # shape: torch.Size([256, 64, 5])
        # masked_input
        # shape: torch.Size([256, 64, 5])
        ...
        # 使用 [CLS] token 作为全局表示
        # 使用平均池化聚合整个序列的信息
        out_view1 = F.normalize(out_view1, dim=2)  # [B, T, D]
        # 应用投影头
        proj_view1 = self.projection_head(out_view1)  # [B, T, 256]
        proj_view1 = F.normalize(proj_view1, dim=2)  # [B, T, 256]
        # 预测头
        pred_view1 = self.predictor(proj_view1)  # [B, T, 256]
        ...
        with torch.no_grad():
            self.momentum_encoder.update_encoder_k(self.bert)
            out_view2, hidden_states2, _ = self.momentum_encoder.encoder_k(x=contra_view2, padding_masks=padding_masks2,
                                                                           batch_temporal_mat=batch_temporal_mat2,
                                                                           argument_methods=argument_methods2,
                                                                           graph_dict=graph_dict,
                                                                           output_hidden_states=output_hidden_states,
                                                                           output_attentions=output_attentions)
            out_view2 = F.normalize(out_view2, dim=2)  # [B, T, D]
            proj_view2 = self.projection_head(out_view2)  # [B, T, 256]
            proj_view2 = F.normalize(proj_view2, dim=2)  # [B, T, 256]



        contrastive_loss = F.mse_loss(pred_view1, proj_view2)




        # a = self.mask_l(bert_output)
        return pred_view1, proj_view2, MLM_predictions,contrastive_loss

#
# def global_contrastive_loss(proj_view1, proj_view2, temperature=0.07):
#     """
#     计算全局对比损失，利用所有 token 的表示。
#
#     Args:
#         proj_view1: [B, T, D]
#         proj_view2: [B, T, D]
...
#         loss: 标量损失
#     """
#     B, T, D = proj_view1.shape
#
#     # 归一化
#     proj_view1 = F.normalize(proj_view1, dim=2)  # [B, T, D]
#     proj_view2 = F.normalize(proj_view2, dim=2)  # [B, T, D]
#
#     # 重塑为 [B*T, D]
#     proj_view1 = proj_view1.view(B * T, D)  # [B*T, D]
#     proj_view2 = proj_view2.view(B * T, D)  # [B*T, D]
#
#     # 计算相似度矩阵
#     logits = torch.matmul(proj_view1, proj_view2.T)  # [B*T, B*T]
#     logits /= temperature
#
#     # 创建标签，对角线为正样本
#     labels = torch.arange(B * T).to(proj_view1.device)
#
#     # 计算 InfoNCE 损失
...
class MomentumEncoder(nn.Module):
    ...
    @torch.no_grad()
    def update_encoder_k(self, online_encoder):
