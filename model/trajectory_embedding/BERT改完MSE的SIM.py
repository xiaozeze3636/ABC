...
class BERTContrastiveLM(nn.Module):
    ...
    def forward(self, contra_view1, contra_view2, argument_methods1,
        ...
        # 或者使用 torch.log 和添加 epsilon
        # mi_lower_bound1 = torch.mean(T_pos1[:, 0]) - torch.log(torch.mean(torch.exp(T_neg1[:, 0])) + epsilon)
        # mi_lower_bound2 = torch.mean(T_pos2[:, 0]) - torch.log(torch.mean(torch.exp(T_neg2[:, 0])) + epsilon)

        mi_lower_bound =(mi_lower_bound1 + mi_lower_bound2)/2
    ...
    def simsiam_loss(self, p, z):
        ...
        loss = F.mse_loss(p, z, reduction='mean')
        return loss
