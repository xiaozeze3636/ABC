...
def contrastive_loss(self, pool_out_view1):
    """
    计算对比损失并更新对照队列
    Args:
        pool_out_view1: (B, D) 主编码器生成的查询嵌入
    Returns:
        contrastive_loss: 对比损失值
    """
    # **计算查询与队列中所有键的相似度**
    logits = torch.mm(pool_out_view1, self.queue.queue) / self.temperature  # (B, queue_size)

    # **生成标签，假设每个查询对应队列中的一个正样本**
    # 这里需要确保队列中的第一个 B 个键是正样本对应的
    labels = torch.arange(pool_out_view1.size(0)).to(self.device)

    # **计算交叉熵损失**
    loss = F.cross_entropy(logits, labels)

    # **将当前查询嵌入添加到对照队列中**
    self.queue.enqueue(pool_out_view1.detach())
    ...
    def _valid_epoch(self, eval_dataloader, epoch_idx, mode='Eval'):
        ...
        with torch.no_grad():
            for i, batch in tqdm(enumerate(eval_dataloader), desc="{} epoch={}".format(
                ...
                mean_loss_l, batch_loss_l, num_active_l = self._cal_loss(predictions_l, targets_l, target_masks_l)
                # mean_loss_con = self._contrastive_loss(z1, z2, self.contra_loss_type)
                mean_loss_con = self.model.contrastive_loss(z1)
