...
class BertBaseExecutor(AbstractExecutor):
    ...
    # 通过给定数据集，在某个训练周期内，进行计算损失和准确率，并且记录结果
    ...
    def _valid_epoch(self, eval_dataloader, epoch_idx, mode='Eval'):
        ...
        epoch_loss = 0  # total loss of epoch
        total_correct = 0  # total top@1 acc for masked elements in epoch 总的正确预测的数目
        total_active_elements = 0  # total masked elements in epoch 几个被盖住了
