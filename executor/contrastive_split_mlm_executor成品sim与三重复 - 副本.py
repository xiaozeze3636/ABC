...
class ContrastiveSplitMLMExecutor(ContrastiveMLMExecutor):
    ...
    def _train_epoch(self, train_dataloader, epoch_idx):
        ...
        for i, batch in tqdm(enumerate(train_dataloader), desc="Train epoch={}".format(
            ...
            loss_simsiam, predictions_l,  mi_lower_bound = self.model(contra_view1=contra_view1, contra_view2=contra_view2,
            ...
            # print(mean_loss_l)
            # print(byol_loss)
            mean_loss = self.mlm_ratio * mean_loss_l + self.contra_ratio * loss_simsiam - mi_lower_bound
            # print(mean_loss)
            ...
            if batches_seen % self.grad_accmu_steps == 0:
                ...
                # # 更新动量编码器
                # self.model.momentum_encoder.update_encoder_k(self.model.bert, self.model.projection_head)
                #
                # # 验证动量编码器更新
                # with torch.no_grad():
                #     encoder_diff = 0.0
                #     for param_q, param_k in zip(self.model.bert.parameters(),
                #                                 self.model.momentum_encoder.encoder_k.parameters()):
                #         encoder_diff += torch.norm(param_q - param_k).item()
                #     projection_diff = 0.0
                #     for param_q, param_k in zip(self.model.projection_head.parameters(),
                #                                 self.model.momentum_encoder.projection_head_k.parameters()):
                #         projection_diff += torch.norm(param_q - param_k).item()
                    # self._logger.info(
                    #     f"Encoder parameter difference after update: {encoder_diff:.6f}, Projection parameter difference: {projection_diff:.6f}")
            ...
            post_fix = {
                ...
                "Bmi_lower_bound": mi_lower_bound.item(),
                "loss_simsiam": loss_simsiam.item(),
            ...
            # 添加动量系数的当前值（如果需要）
            # post_fix['momentum'] = self.model.momentum_encoder.momentum
